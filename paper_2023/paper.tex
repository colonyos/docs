...
Colonies can be used to implement a decentralized, grid-like compute infrastructure for executing EO workloads across diverse platforms. By encapsulating computational EO tasks in Colony processes, interoperability among platforms can be significantly enhanced. Executors could also be used to provision computational resources if needed. For example, a provisioning engine similar to serverless engine presented in \ref{faas} can be used to deploy extra cloud executors to speed up execution of demaning EO workloads, or take advantage of HPC resources as discussed in Section \ref{hpc_use_case}. 

\section{Discussion}
This paper has presented an open-source framework called Colonies, which streamlines execution of computational workloads across platforms. Colonies enables creation of a grid-like distributed virtual computer, called a \emph{Colony}, which is a collective machine consisting of distributed executors running on the Internet. Unlike many other systems where security sometimes is added later on, Colonies builds on the principle, \emph{security first}.
...
As a final remark, it is worth highlighting once again the significant advantage of Colonies as a security solution. As a colony is a trusted community of computer software, Colonies can be used to share secrets and credentials among its executor members. By storing credentials, such as one-time passwords or authentication keys in process records, executors can just execute a process in run-and-forget style and be able to access third-party infrastructures without the need to store the and manage credentials locally. Additionally, by targeting specific executor types when submitting a function specification, a colony can be further segmented so that only a subset of the executors in a colony can access the credentials, further improving the security. 
