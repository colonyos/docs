...
Another features of the Colonies framework are so-called generators. Generators are workflow templates that are triggered automatically when specific conditions are met. Generators can be used accumulate input data in bulks to improve performance or parallelism. For example, an external service downloads satellite images and a workflow should automatically be triggered when 10 images are collected. This is achieved by sending a special \emph{pack request} containing input data to a Colonies server, targeting a certain generator. 
