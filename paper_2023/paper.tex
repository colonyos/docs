...
To address these challenges, the paper presents an open-source framework called Colonies\footnote{https://github.com/colonyos/colonies}, which facilitates execution of computational workloads across a diverse range of platforms. Colonies enables development of a \emph{decentralized virtual computer system}, called a \emph{Colony}, which can effortlessly integrate with any third-party application or other workflow systems.
...
\begin{figure}[h]
	...
	\caption{Using Colonies to control drones.}
	\label{fig:dashboard2}
\end{figure}
...
\section{Discussion}
This paper has presented an open-source framework called Colonies, which is designed to streamline execution of computational workloads across platforms. Colonies enable the creation of a \emph{decentralized virtual computer system}, called a \emph{Colony}, which serves as an intermediary layer to indirectly control and integrate with other computer systems. Instead of directly connecting different systems togeather, Colonies allows machines or users to publish instructions and then assign the instructions to other machines. By chaining these instructions together, it becomes possible to execute workloads that can operate seamlessly across different platforms. 
...
A significant portion of the paper has focused on describing how to handle robustness and allow software components to fail without compromising the entire system. A critical design decision that makes Colonies robust and scalable is its stateless architecture. Colonies also makes sure that workflow execution run to completion, even if some executor failes. This makes it possible to implement robust data pipelines, but also to efficiently handle IT operations such as Continuous Deployments (CD) and software upgrades. Both security and robustness are crucial factors to consider when developing a workflow management system that is capable of functioning across platforms and handle constantly changing infrastructures. 

It has been mentioned in the paper several times that Colonies is designed to simplify integration with other systems, but what feature in Colonies makes integration easier? Microservices is a design principle used by Colonies to simplify integration. As executors are designed to be loosly-coupled and can be implemented in any language (supported by Colonies), the whole system becomes easier to develop and maintain. For example, different teams can take on responsibility to develop a certain executor, which can then be used as a part of complex workflows running across platforms. By designing each executor as a standalone microservice, it becomes easy to scale the system by simply deploying more executors. This also opens up an exciting possibility to delegate colony management responsibility to another software to implement a serverless computing platform capable of automatically provisioning and deploying executors whenever they are needed.

The paper has also presented some potential use cases. In all of the use cases, Colonies serves as intermediary layer that breaks down complex heterogenous systems in to smaller, resilient and more scalable services, and then offer a unified interface for the services to interact with each other. For example, in HPC use case, Colonies can help to simplify HPC workflow managemewnt by breaking down complex perations into a set of functions that can be executed as part of a workflow.


% The paper has presented several potential use cases where Colonies serves as an intermediary layer that decomposes complex heterogeneous systems into smaller, resilient, and scalable services, and then provide a unified interface for seamless interaction between these services. For instance, in the HPC use cases, Colonies can streamline the workflow management process by HPC operations into a collection of functions that can be seamlessly executed as part of a workflow.
