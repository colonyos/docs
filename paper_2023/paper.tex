...
Implementing a queue using a database can be achieved in SQL by utilizing the \emph{order by timestamp} and \emph{limit 1} clauses to select tasks according to their submission times. To make it possible to handle priorites, adjust the submission time so that higher priority processes are processed before lower priority processes. When a process is submitted, a \emph{priority time} value is computed and stored in the database. The primary is a modified submission timestamp calculated by subtracting a delta time calculated as a function of a priority value. This enables prioritization of more critical tasks. Equation \ref{eq:pt} shows how the priority time is calculated for a nanosecond timestamp. 
