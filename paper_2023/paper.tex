...
Although microservices have attractive properties and simplify both development and deployment of executors, they also introduce additional complexity that must be managed by the Colonies servers. To implement a workflow management framework supporting distributed microservices, the following challenges must be addressed:
...
The advantage of Colonies for implementing a FaaS platform is that it introduces an extra level of separation and the possibility to develop a loosely coupled system. For example, if an executor is missing, the process is simply enqueued in the process table. A software agent could then monitor pending processes on the Colonies server, predict the system load, and automatically deploy executors to meet the load, resulting in a highly efficient and scalable system. In the future, the AI orchestrator developed in the Cognit project could also be used together with Colonies to reduce energy consumption or minimize network latency by efficiently utilizing edge servers.
...
Colonies could be used to fill the gap between cloud and HPC and create workflows that can automate processing of data in the cloud while also taking advantage of the vaste computational capacity offered by HPC. For example, a company could develop a workflow that automatically copy training data to a HPC center, train a neural network, move the result back to the cloud, and finally re-deploy a Kubernetes service to use the new model.

Figure \ref{fig:hpc_workflow} shows an example how Colonies could be used in a HPC environment. Several executors run at the HPC system, each with their own responsibilities. The Container Importer Executor is responsible for converting container images to a format that is supported by Slurm, e.g. Singularity containers\footnote{https://docs.sylabs.io}. The Data Importer Executors downloads data to the HPC file system. The Slurm Executor is responsible for submitting and overseeing Slurm jobs, while the Data Exporter executor is responsible for uploading result data to the cloud.

\begin{figure}[h]
	\centering
    \includegraphics[scale=0.43]{hpc_overlay.png}
	\caption{A global grid-like master-control submission system for HPC.}
	\label{fig:hpc_overlay}
\end{figure}

In general, integrating with various authentication services presents a major challenge when developing HPC workflows. For example, the Data Importer may need to access an S3 service to fetch data. The Container Importer must authenticate with a Docker registry to be able to pull a Docker container to the HPC system. In addition, it is not straightforward to connect to the HPC system. In many cases, users have to manually connect using SSH terminals, making it time-consuming and difficult to automate HPC workflows. 

With Colonies, users can indirectly control HPC systems by submitting Colonies processes that are subsequently picked up by HPC executors, without requiring direct connection to the HPC system. Essentially, using Colonies as a cloud-based submission system for HPC, connecting different HPC centers together in a grid-like overlay network. Figure \ref{fig:hpc_overlay} depicts such a submission system.

Additionally, the security model provided by Colonies allows different executors to trust one another, thereby enabling the sharing of confidential information, such as S3 secrets. As credentials are stored in the assigned process record, there is no need to store them locally on the HPC system. As a result, HPC executors become more straightforward to configure and deploy, further simplifying creation of HPC workflows.
